								...
								.each(
										...
										function(kk, vv) {
											...
											if (kk == 0) {
												...
												props
														.push('<th scope="row" rowspan="'
																+ v.length
																+ '" ><div class="card" style="width: 200px;"><img class="card-img-top" src="'
																+ path
																+ vv.test.logoLangage
																+ '" alt="Logo du langage de programmation"><div class="card-body"><h5 class="card-title">');
												props.push(vv.test.libelle
														+ '</div></div></th>');
											}
											console.log(vv);
											props.push('<td>' + vv.candidat.nom
													+ '</td>');
											props.push('<td>'
													+ vv.candidat.prenom
													+ '</td>');
											props.push('<td>' + vv.noteCandidat
													+ '/20</td>');
											if (vv.niveauCandidat == 'Non acquis') {
												props
														.push('<td class="table-danger">'
																+ vv.niveauCandidat
																+ '</td>');
											} else if (vv.niveauCandidat == 'Acquis') {
												props
														.push('<td class="table-success">'
																+ vv.niveauCandidat
																+ '</td>');
											} else {
												props
														.push('<td class="table-warning">'
																+ vv.niveauCandidat
																+ '</td>');
											}
...
function afficherListeTestUtil(xml) {
	...
	if (json.epreuves.length > 0) {
		...
		for (x = 0; x < json.epreuves.length; x++) {
			...
			props
					.push('<div class="col-lg-3 col-md-4 col-xs-6"><div class="card" style="width: 200px;"><img class="card-img-top"	src="'
							+ path
		...
		document.getElementById("tests").innerHTML = '<div class="row justify-content-center"><h3>Vous n\'avez terminé aucun test</h3></div>'
	}
}