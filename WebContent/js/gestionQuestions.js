...
function checkPropositions() {
	...
	if (enonce == '') {
		echec(1, "Vous n'avez pas mis l'énoncé");
	} else {
		...
		if (themesChoisis.length = 0) {
			echec(1, "Sélectionnez au moins 1 thème");
		} else {
			var points = document.getElementById('points').value;
			if (points == '') {
				echec(1, "Rentrez le nombre de points")
			} else {
				var z;
				var propositions = '' + prop1 + '---' + prop2 + '---' + prop3
						+ '---' + prop4 + '---' + prop5;
				var bonnesReponses = '' + check1 + '---' + check2 + '---'
						+ check3 + '---' + check4 + '---' + check5;
				var Themes = '';
				for (z = 0; z < themesChoisis.length; z++) {
					Themes = Themes + themesChoisis[z].value + '---';
				}
				data = "&enonce=" + encodeURIComponent(enonce)
						+ "&propositions=" + encodeURIComponent(propositions)
						+ "&bonnesReponses="
